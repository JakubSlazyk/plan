...
public class OsobyController {
    ...
    @GetMapping(value = "/osoby", produces = { MediaType.APPLICATION_JSON_VALUE })
    ...
    @PostMapping(value = "/osoby", produces = { MediaType.APPLICATION_JSON_VALUE })
    ...
    private JSONObject getPlanFromFile(String path) {
        ...
        try {
            FileReader reader = new FileReader(path + "/plan/osoby.json");
    ...
    private boolean savePlanToFile(String path, JSONObject jsonObject) {
        try (FileWriter file = new FileWriter(path + "/plan/osoby.json")) {
